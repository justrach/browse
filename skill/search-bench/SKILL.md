...
name: search-bench
description: >
  Drive the SEACHAI macOS browser from the repo-root ./bench command: open
  ...
  press Chrome extensions. Use when the user asks to test SEACHAI, drive the
...
metadata:
  short-description: Drive SEACHAI with ./bench
---

# SEACHAI bench

SEACHAI is a native Mac browser in this repo. Drive the running app with `./bench` from the repo root. `./bench help` is the command syntax. This file is how to use it without touching the browser a person has open.
...
Use a test world for any work that changes chrome, installs or removes extensions, resizes, sends real key events, or selects a tab. A `swift build` binary under `.build/` is always world `test`, even with no `SEARCH_PROBE`. `./fresh.sh` launches `build/SEACHAI.app` with `SEARCH_PROBE` set, which is also a test world.
...
One process per world. Quit a process only after its executable path is this repo's `build/SEACHAI.app` or a `.build/` binary, or its environment contains `SEARCH_PROBE`. Leave installed browser apps alone. `killall` and `osascript` quit can hit an installed app too.
...
4. The suite is `com.codegraff.search.test`, or `com.codegraff.search.test.NAME` for a named world.
...
If `./bench tabs` (no flag) is not listening, ask them to turn on **Settings › General › Let a script drive SEACHAI**. Do not write defaults for the installed app.

`./fresh.sh` builds `build/SEACHAI.app` when that bundle is missing.
